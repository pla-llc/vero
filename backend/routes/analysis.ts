...
const app = createHono()
	...
	.post(
		"/tokens/analyze-sync",
		schemaValidator("json", analysisRequestSchema),
		async (c) => {
			try {
				...
				const { getTrendingTokensTool, getNewListingsTool } =
					await import("../lib/mastra");

				// Step 1: Fetch trending tokens
				console.log(
					`[Analysis Debug] Fetching trending tokens with limit: ${validated.trendingLimit}`
				);
				const trendingResult = await (
					getTrendingTokensTool as any
				).execute({
					input: {
				...
				const newListingsResult = await (
					getNewListingsTool as any
				).execute({
					input: {
				...
				if (!trendingResult.success && !newListingsResult.success) {
					throw new Error(
						"Failed to fetch both trending tokens and new listings"
					);
				}
				...
				const uniqueTokens = allTokens.filter(
					(token, index, self) =>
						index ===
						self.findIndex((t) => t.address === token.address)
				);
