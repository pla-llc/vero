...
const app = createHono()
  ...
  .get("/balance/:token", async (c) => {
    try {
      ...
      const balance = await WalletService.getTokenBalance(session.user.id, tokenMint);
  ...
  .post("/balances", async (c) => {
    try {
      ...
      const tokenMints = tokens.map((token: string) => {
        return TOKENS[token.toUpperCase() as keyof typeof TOKENS] || token;
      });

      const balances = await WalletService.getMultipleTokenBalances(session.user.id, tokenMints);
  ...
  .post("/swap/quote", async (c) => {
    try {
      ...
      const { inputToken, outputToken, amount, slippage } = await c.req.json();
      
      let inputMint = TOKENS[inputToken.toUpperCase() as keyof typeof TOKENS];
      let outputMint = TOKENS[outputToken.toUpperCase() as keyof typeof TOKENS];
  ...
  .get("/saved-wallets", async (c) => {
    try {
      ...
      const wallets = await WalletService.getSavedWallets(session.user.id);
  ...
  .post("/saved-wallets", async (c) => {
    try {
      ...
      const wallet = await WalletService.addSavedWallet(session.user.id, label, address);
      return c.json({ wallet });
    } catch (error) {
      console.error("Error adding saved wallet:", error);
      return c.json({ error: error.message || "Failed to add wallet" }, 500);
    }
  })
  ...
  .post("/send", async (c) => {
    try {
      ...
      const result = await WalletService.sendTokens(session.user.id, toAddress, token || "SOL", amount);
      return c.json(result);
    } catch (error) {
